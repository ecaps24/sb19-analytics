@echo off
REM YouTube EMOJI MV - X Post Automation (runs every 30 min)
REM Posts EMOJI YouTube stats (views, likes, comments) to X
...
set LOG_FILE=yt_emoji_post.log

echo ============================================ >> %LOG_FILE%
echo [%date% %time%] Starting YouTube EMOJI post... >> %LOG_FILE%
...
python social_media_agent.py youtube-emoji >> %LOG_FILE% 2>&1
if %ERRORLEVEL% NEQ 0 (
    python notify.py "FAILED: YouTube EMOJI post at %date% %time%"
)

@echo off
REM OPM Top 10 Daily - X Post Automation (runs daily at 12:35pm)
REM Posts OPM Top 10 Artists by Monthly Listeners to X
...
set LOG_FILE=opm_top_post.log

echo ============================================ >> %LOG_FILE%
echo [%date% %time%] Starting OPM Top 10 post... >> %LOG_FILE%
...
python social_media_agent.py opm-top >> %LOG_FILE% 2>&1
if %ERRORLEVEL% NEQ 0 (
    python notify.py "FAILED: OPM Top 10 post at %date% %time%"
)
